    ...
    return (
      <div className="voice-of-client">
        <div className="info row-flex space-between">
          <div className="buyer">
            <div>
              <b className="strong-color">{testimonial.name}</b>
            </div>
            <div>
              <i className="fa fa-check-circle m-right-7" />
              Verified Buyer
            </div>
            <div>
              <img
                className="m-right-7"
                src={getImageFlagCountry(String(testimonial.country))}
          ...
          <div>
            <div className="testimonial-satisfield">
              <i className="fa fa-thumbs-up" /> Satisfied
            </div>
        ...
        <div className="client-photos">
          <Carousel controls={testimonial.images.length > 1} interval={null}>
            {testimonial.images.length
              ? testimonial.images.map((image, index) => {
                  return (
                    <Carousel.Item key={index}>
                      <img alt="client" src={getImageCDN(image, 640, 0, 'https')} />
                    </Carousel.Item>
                  )
                })
              : null}
        ...
        <div className="purchased-car row">
          <div className="col-xs-12">Purchased car before delivery (on website)</div>
          ...
          <div className="col-xs-6 purchased-car-detail">
            <div>
              <Link href={testimonial.car.url}>
                <b className="strong-color">{testimonial.car.title}</b>
              </Link>
            </div>
