    ...
    return (
      <div className="voice-of-client-list">
        <h2 className="margin-xs strong-color">
          <b>Voices Of Clients</b>
        </h2>
        <hr />
        <div className="row">
          <div className="col-xs-6">{this.listLeft()}</div>
          <div className="col-xs-6">{this.listRight()}</div>
        </div>
        <div className="text-center">
          {!this.state.showAll && (
            <button className="btn btn-show-more" onClick={this.hanldeShowMore}>
              Show more
            </button>
          )}
          {this.state.showAll && (
            <button className="btn btn-show-all" onClick={this.hanldeShowAll}>
              <a href="/testimonials">Show All</a>
            </button>
          )}
        </div>
      </div>
